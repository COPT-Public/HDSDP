...
for iter = 1:ncorr
    ...
    if dratemax == 0.0
        ...
        gammafeas = dsdpgetalpha(Shat, Ry - dsdpgetATy(A, d4));
        
        slhat = yhat + bound; suhat = bound - yhat;
        dslhat = d4; dsuhat = -d4;
        step = - 1 / min(dslhat ./ slhat);
        if step > 0
            gammafeas = min(gammafeas, step);
        end % End if
        
        step = - 1 / min(dsuhat ./ suhat);
        if step > 0
            gammafeas = min(gammafeas, step);
        end % End if
        
        drate = min(gammafeas * dratemax / alpha, 1.0);
