...
for i = 1:maxiter
    ...
    for newmu = mu
        ...
        if ~ isinf(newpObj)
            if pObj ~= initpObj
                ...
                muprimal = (pObj - dObj - Rd(1, 1) / tau * 1e+10) / (np * 3);
                mu = min(mu, muprimal);
            else
                pObj = newpObj / tau;
                muprimal = (pObj - dObj - Rd(1, 1) / tau * 1e+10) / (np * 3);
    ...
    if (true)
        ...
        gammafeas = dsdpgetalpha(Shat, Rd - dsdpgetATy(A, d4));
        drate = min(gammafeas * 0.98 / alphac, 1.0);
        b2 = -d3 + drate * d4;
    end % End if
...
%     end % End if 
    ...
    % Corrector
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    [y, S, Rd] = dinfeaspotrdc(A, b * tau, C * tau, y, S, Rd, M, d2 * tau, mu, 12, bound, 0.8);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
