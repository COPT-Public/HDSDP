...
extern DSDP_INT vec_alloc( vec *x, const DSDP_INT n ) {
    ...
    return (x) ? DSDP_RETCODE_OK : DSDP_RETCODE_FAILED;
}

extern void vec_copy( vec *src, vec *dst ) {
    // Copy src to dst
    CHECKVEC(src, dst); copy(&(src->dim), src->x, &one, dst->x, &one);
}

extern void vec_axpy( double alpha, vec *x, vec *y ) {
    // Compute y = alpha * x + y
    CHECKVEC(x, y); axpy(&(x->dim), &alpha, x->x, &one, y->x, &one);
}
...
extern void vec_zaxpby( vec *z, double alpha, vec *x, double beta, vec *y ) {
    ...
    CHECKVEC(z, x); CHECKVEC(x, y);
    vec_reset(z);
    if (alpha) { axpy(&(x->dim), &alpha, x->x, &one, z->x, &one); }
    if (beta) { axpy(&(y->dim), &beta,  y->x, &one, z->x, &one); }
}
...
extern double vec_step( vec *s, vec *ds, double beta ) {
    ...
    if (beta == 0.0) { return DSDP_INFINITY; }
    if (beta > 0.0) {
        for (DSDP_INT i = 0; i < s->dim; ++i) {
            tmp = ds->x[i] / s->x[i]; alpha = MIN(tmp, alpha);
        }
        alpha = -1.0 / (alpha * beta);
    } else {
        alpha = - DSDP_INFINITY;
        for (DSDP_INT i = 0; i < s->dim; ++i) {
            tmp = ds->x[i] / s->x[i]; alpha = MAX(tmp, alpha);
        }
        alpha = -1.0 / (alpha * beta);
    }

    if (alpha <= 0.0) { alpha = DSDP_INFINITY; }
    return alpha;
}
