...
static DSDP_INT isSparseRank1( spsMat *dataMat, DSDP_INT *isRank1 ) {
    ...
    DSDP_INT n = dataMat->dim, col = 0, isNeg = FALSE, nnz = 0, isR1 = TRUE;
    ...
    if (Ai[0] != col) {
        isR1 = FALSE; *isRank1 = isR1; return retcode;
    }
    
    double *a = (double *) calloc(n, sizeof(double)), adiag = Ax[0];
    if (adiag < 0) {
    ...
    for (DSDP_INT j = Ap[col]; j < Ap[col + 1]; ++j) {
        ...
        a[Ai[j]] = Ax[j] / adiag; nnz += (Ax[j] != 0);
    }
    
    if (dataMat->nnz != (DSDP_INT) (nnz + 1) * nnz / 2) {
        isR1 = FALSE; // Currently only nnz is checked and there is still chance of mismatch
    } else {
        // Ready to check rank-one property
        if (isNeg) {
            for (DSDP_INT i = 0; i < n; ++i) {
                for (DSDP_INT j = Ap[i]; j < Ap[i + 1]; ++j) {
                    diff = Ax[j] + a[i] * a[Ai[j]]; err += diff * diff;
                }
                if (err > 1e-06) {
                    isR1 = FALSE; break;
                }
...
static DSDP_INT extractR1fromSps( spsMat *dataMat, double *a, DSDP_INT isNeg ) {
    ...
    DSDP_INT retcode = DSDP_RETCODE_OK;
    DSDP_INT n = dataMat->dim, *Ap = dataMat->p, *Ai = dataMat->i;
    DSDP_INT col = 0; double adiag = 0.0, *Ax = dataMat->x;
    ...
    for (DSDP_INT j = Ap[col]; j < Ap[col + 1]; ++j) {
        a[Ai[j]] = Ax[j] / adiag; // If the diagonal is zero but other rows contain non-zeros
    }
...
static DSDP_INT preRank1RdcBlock( sdpMat *dataMat ) {
    ...
    for (DSDP_INT i = 0; i < m + 1; ++i) {
        ...
        switch (types[i]) {
            ...
            case MAT_TYPE_DENSE: isDenseRank1Acc((dsMat *) matdata[i], &isRank1); isDense = TRUE; break;
            case MAT_TYPE_SPARSE: isSparseRank1((spsMat *) matdata[i], &isRank1); isSparse = TRUE; break;
            case MAT_TYPE_RANKK: break;
            default: error(etype, "Unknown matrix type. \n"); break;
        }
        
        // If rank-one structure is detected, free current structure and use rank-one structure
        if (isRank1) {
            ...
            if (isDense) {
                ...
                denseMatFree(dsdata); DSDP_FREE(dsdata);
                dataMat->ndenseMat -= 1;
            }
...
static DSDP_INT preSDPMatgetPScaler( HSDSolver *dsdpSolver ) {
    ...
    vec_init(dsdpSolver->pScaler); vec_alloc(dsdpSolver->pScaler, m);
...
static DSDP_INT preSDPgetSymbolic( HSDSolver *dsdpSolver, DSDP_INT blockid ) {
    ...
    DSDP_INT dim = sdpBlock->dimS, nhash = nsym(dim), nnz = 0;
    ...
    // Get hash table
    DSDP_INT *hash = (DSDP_INT *) calloc(nhash, sizeof(DSDP_INT));
    ...
    if (useDenseS) { nnz = nhash;
    } else {
        // Construct hash table by the symbolic features
        DSDP_INT idx = 0;
        for (DSDP_INT i = 0; i < nhash; ++i) {
            if (hash[i]) { hash[i] = idx; idx += 1; }
        }
        assert( idx == nnz );
    }
    ...
    if (isfirstNz && !useDenseS) {
        ...
        dsdpSolver->S[blockid]->nzHash[tmp] = 0; tmp += 1;
    }
    
    for (DSDP_INT i = 0; i < dim; ++i) {
        for (DSDP_INT j = i; j < dim; ++j) {
            if (useDenseS) {
                dsdpSolver->S[blockid]->i[tmp] = j; tmp += 1;
            } else if (packIdx(hash, dim, j, i)) {
                dsdpSolver->S[blockid]->i[tmp] = j;
                dsdpSolver->S[blockid]->nzHash[tmp] = i; tmp += 1;
            }
        }
    ...
    memcpy(dsdpSolver->dS[blockid]->p, dsdpSolver->S[blockid]->p, sizeof(DSDP_INT) * (dim + 1));
    memcpy(dsdpSolver->Scker[blockid]->p, dsdpSolver->S[blockid]->p, sizeof(DSDP_INT) * (dim + 1));
    memcpy(dsdpSolver->dS[blockid]->i, dsdpSolver->S[blockid]->i, sizeof(DSDP_INT) * tmp);
    memcpy(dsdpSolver->Scker[blockid]->i, dsdpSolver->S[blockid]->i, sizeof(DSDP_INT) * tmp);
    memcpy(dsdpSolver->dS[blockid]->x, dsdpSolver->S[blockid]->x, sizeof(double) * tmp);
    memcpy(dsdpSolver->Scker[blockid]->x, dsdpSolver->S[blockid]->x, sizeof(double) * tmp);
    ...
    if (useDenseS) {
        ...
        memcpy(dsdpSolver->dS[blockid]->nzHash, dsdpSolver->S[blockid]->nzHash, sizeof(DSDP_INT) * nnz);
        memcpy(dsdpSolver->Scker[blockid]->nzHash, dsdpSolver->S[blockid]->nzHash, sizeof(DSDP_INT) * nnz);
    }
...
extern DSDP_INT preLPMatScale( lpMat *lpData, vec *lpObj, vec *pScaler ) {
    ...
    double maxNrm = 0.0, minNrm = 0.0, tmp = 0.0;
    ...
    for (DSDP_INT i = 0; i < n; ++i) {
        ...
        for (DSDP_INT j = Ap[i]; j < Ap[i + 1]; ++j) {
            tmp = Ax[j] / scal[Ai[j]];
            Ax[j] = tmp; maxNrm = MAX(maxNrm, tmp); minNrm = MIN(minNrm, tmp);
        }
