...
shadedErrorBar([1, batchrange], mean(sgdSpeedup), std(sgdSpeedup), "lineProps", {"-d",  "LineWidth", 3, "Color", [0.13, 0.55, 0.13]});
% plot([1, batchrange], proxlinSpeedup, "-x",  "LineWidth", 3);
hold on;
shadedErrorBar([1, batchrange], mean(proxlinSpeedup), std(proxlinSpeedup), "lineProps", {"-o",  "LineWidth", 3, "Color", [0.12, 0.56, 1.00]});
...
legend(["SGD", "SPL"], "FontSize", 20);
% xlabel("batchsize m");
% ylabel("speedup");
set(gca, "Fontsize", 20);
savefig("no_spp_blind_kappa_" + kappa + "_pfail_" + pfail + "_type_1.fig")
fprintf("Done. \n");
...
for i = 1:nrobusttest
    ...
    shadedErrorBarSemi(steprange, mean(robustproxlinSpeedup), std(robustproxlinSpeedup), "lineProps", {"-d", "LineWidth", 3, "Color", [0.13, 0.55, 0.13]});
    hold on;
    % semilogx(steprange, robustsgdSpeedup, "-o", "LineWidth", 3);
    shadedErrorBarSemi(steprange, mean(robustsgdSpeedup), std(robustsgdSpeedup), "lineProps", {"-o", "LineWidth", 3, "Color", [0.12, 0.56, 1.00]});
    hold on;
    ...
    legend(["SGD", "SPL"], "FontSize", 20);
    set(gca, "Fontsize", 20);
    savefig("no_spp_blind_kappa_" + kappa + "_pfail_" + pfail + "_type_2_batch_" + batchsize + ".fig")
...
for i = 1:nrobusttest
    ...
    shadedErrorBarSemi(steprange, nproxlinIterToOpt(idx, :), ...
        std(transpose(reshape(nproxlinIter(idx, :, :), nsteptotest, nTest))), "lineProps", {"-d", "LineWidth", 3, "Color", [0.13, 0.55, 0.13]});
    % semilogx(steprange, nproxlinIterToOpt(idx, :), "-x", "LineWidth", 3);
    hold on;
    shadedErrorBarSemi(steprange, nsgdIterToOpt(idx, :), ...
        min(std(transpose(reshape(nsgdIter(idx, :, :), nsteptotest, nTest))), nsgdIterToOpt(idx, :)), "lineProps", {"-x", "LineWidth", 3, "Color", [0.12, 0.56, 1.00]});
    % semilogx(steprange, nsgdIterToOpt(idx, :), "-o", "LineWidth", 3);
    hold on;
%     shadedErrorBarSemi(steprange, nproxptIterToOpt(idx, :), ...
%         std(transpose(reshape(nproxptIter(idx, :, :), nsteptotest, nTest))), "lineProps", {"-x", "LineWidth", 3, "Color", "r"});
    ...
    set(gca, "Fontsize", 20);
    ylim([0, 6e+04]);
    savefig("no_spp_blind_kappa_" + kappa + "_pfail_" + pfail + "_type_3_batch_" + batchsize + ".fig")
